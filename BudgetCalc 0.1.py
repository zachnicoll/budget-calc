...
class removeWindow(object):
     def __init__(self, master):
        ...
        Label(F_title, text = "DELETE A CATEGORY", font=('Castellar',16,'bold')).grid(column = 0, row = 0)
        ...
        del_B = Button(F_input, text = "DELETE", command = lambda: delCall(), font=('Arial Black',14,'bold'), bg = "dark blue", fg="white")
        del_B.grid(column = 0, row = 1, pady = pady)
        cancel_B = Button(F_input, text = "BACK", command = lambda: cancelCall(), font=('Arial Black',14,'bold'), bg = "red", fg="white")
...
class addWindow(object):
    def __init__(self, master):
        ...
        F_title = Frame(master)
        F_title.grid(column = 0, row = 0)
        Label(F_title, text = "ADD NEW CATEGORY", font=('Castellar',16,'bold')).grid(column = 0, row = 0)
        ...
        add_B = Button(F_input, text = "+ADD", command = lambda: addCatCall(), font=('Arial Black',14,'bold'), bg = "dark green", fg="white")
...
class manageWindow(object):
    def __init__(self, master):
        ...
        F_title = Frame(master)
        F_title.grid(column = 0, row = 0)
        Label(F_title, text = "MANAGE CATEGORIES", font=('Castellar',16,'bold')).grid(column = 0, row = 0)
        ...
        for i in range(len(params.u_Params)):
            ...
            if(i == len(params.u_Params)-1):
                ...
                addCat_B = Button(F_input, text = "+ ADD NEW", command = lambda: addCall(), font=('Arial Black',14,'bold'), bg = "dark green", fg="white")
                addCat_B.grid(column = 0, row = i+1, pady = pady)
                dltVal_B = Button(F_input, text = "REMOVE", command = lambda: removeCall(), font=('Arial Black',14,'bold'), bg = "dark blue", fg="white")
...
class mainWindow(object):
    ...
    def __init__(self, master):
        ...
        for i in range(len(params.u_Params)):
            ...
            #params.u_Rmndr[i] = params.u_Rmndr[i] + params.u_Values[i]
            ...
            if(i == len(params.u_Params)-1):
                ...
                Label(F_input, text = "$" + str(round(sum(params.u_Rmndr), 2)) + "/" + str(round(params.u_Goal, 2))).grid(column = 2, row = i+1, pady = pady)
        ...
        def calcCall():
            ...
            for i in range(len(params.u_Params)):
                if(params.entries[i].get() != ""):
                    ...
                    params.rmndrs[i].grid(column = 2, row = i, padx = padx, pady = pady)
                if(i == len(params.u_Params)-1):
                    Label(F_input, text = "$" + str(round(sum(params.u_Rmndr), 2)) + "/" + str(params.u_Goal)).grid(column = 2, row = i+1, pady = pady)
                params.entries[i].delete(0, 100)

            write()

        calc_B = Button(F_input, text = "CALCUALTE", command = lambda: calcCall(), font=('Arial Black',14,'bold'), bg = "dark green", fg="white")
        calc_B.grid(column = 0, row = len(params.u_Params)+1, pady = 5, columnspan = 2)

        manage_B = Button(F_input, text = "MANAGE", command = lambda: self.manageCall(), font=('Arial Black',14,'bold'), bg = "dark red", fg="white")
